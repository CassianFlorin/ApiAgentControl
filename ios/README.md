...
自动重试**只有 3 次**（1/2/4 秒退避），覆盖切网、锁屏这类瞬时抖动；
连续失败就停在「已断开」，由你点重连。

不做无限自动重连是刻意的：它给人的是"看起来在恢复"的假象。
实测遇到过 daemon 侧房间记账出错，客户端一直重连也无济于事，
而界面始终显示"连接中"，反而掩盖了真实状态。

重连入口有两处：左上角的连接徽章（断开时它本身就是按钮），
以及会话为空时的整页提示。设置页也有一个。
